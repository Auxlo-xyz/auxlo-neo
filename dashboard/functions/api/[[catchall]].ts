...
app.get('/api/somnia/balance', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
...
app.get('/api/somnia/activity', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
...
app.get('/api/admin/scan-targets', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
...
app.post('/api/admin/scan-targets', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
...
app.delete('/api/admin/scan-targets/:target', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
...
app.post('/api/admin/scan-now', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
...
app.get('/api/admin/crons', async (c) => {
  const user = await requireAuth(c)
  if (!user) return c.json({ error: 'Unauthorized' }, 401)
